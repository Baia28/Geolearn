...
class PassiveReviewView(ft.View):
    def __init__(self, master_sheet: dict, unit_title: str, on_back: callable, play_audio: callable):
        ...
        pairs_list = ft.ListView(expand=1, spacing=15, padding=20)
        ...
        for lesson_num, content in master_sheet.items():
            ...
            # --- Phrases & Pairs ---
            if content['pairs']:
                pairs_list.controls.append(ft.Text(f"Lesson {lesson_num}", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_GREY_800))
                for pair in content['pairs']:
                    prompt = pair['prompt']
                    resp = pair['correct_response']
                    pairs_list.controls.append(
                        ft.Card(
                            elevation=1, surface_tint_color=ft.Colors.WHITE,
        ...
        tabs.tabs.extend([
            ft.Tab(text="📖 Vocabulary", content=vocab_list),
            ft.Tab(text="💬 Phrases", content=pairs_list),
            ft.Tab(text="🎭 Dialogues", content=dialogues_list)
        ])
